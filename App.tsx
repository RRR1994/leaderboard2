...
import { Plus, X, Camera, MessageSquare, ShieldCheck, Loader2, Trophy, CreditCard, Lock } from 'lucide-react';

type ModalState = 'FORM' | 'PAYPAL_GATEWAY' | 'PROCESSING' | 'CONFIRMED' | 'ERROR';

const App: React.FC = () => {
  // Persistence logic: Load from localStorage or use initial data
  ...
  // PayPal Button Initialization logic
  useEffect(() => {
    if (modalState === 'PAYPAL_GATEWAY' && (window as any).paypal) {
      ...
      (window as any).paypal.Buttons({
        ...
        onApprove: async (data: any, actions: any) => {
          ...
          try {
            ...
            setErrorMessage('Transaction failed to capture. Please try again.');
            setModalState('ERROR');
          }
        },
        onError: (err: any) => {
          console.error('PayPal Error:', err);
          setErrorMessage('PayPal Gateway error. Please check your credentials.');
          setModalState('ERROR');
        }
      }).render(paypalContainerRef.current);
    }
  }, [modalState, newAmount, newName, finalProjectedRank]);
              ...
              <AnimatePresence mode="wait">
                {modalState === 'FORM' && (
                  <motion.div key="form" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }}>
                    <h2 className="text-3xl font-black mb-1 uppercase tracking-tighter">Ascend</h2>
                ...
                {modalState === 'ERROR' && (
                  <motion.div key="error" className="flex flex-col items-center justify-center py-8 text-center">
                    <div className="w-16 h-16 border border-red-500 flex items-center justify-center mb-6 text-red-500 font-black text-2xl">!</div>
                    <h3 className="text-xl font-black uppercase tracking-widest mb-4">Ascension Failed</h3>
                    <p className="text-zinc-500 text-xs mb-8">{errorMessage}</p>
